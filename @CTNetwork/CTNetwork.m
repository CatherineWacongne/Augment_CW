classdef CTNetwork < handle
  ...
  properties
    ...
    xy_weight_range = .30; % Range [-a,a] of initial weights
    yz_weight_range = .20; % Range [-a,a] of initial weights
...
%       obj.Y_ma_current = zeros(1, obj.ny_memory);
  ...
  methods (Access=protected) % Protected methods
    ...
    function init_weights(obj) % Initialize weights
      ...
      obj.weights_xy = obj.xy_weight_range * rand(obj.n_inputs*2+1, obj.ny_normal);%- obj.xy_weight_range;
      obj.weights_xy( 3:2+3*obj.grid_size^2,2:end) = obj.weights_xy( 3:2+3*obj.grid_size^2,2:end).*repmat(W,3,1)+0.05.*repmat(W,3,1);
      obj.weights_xy( 4+3*obj.grid_size^2:end,2:end) = obj.weights_xy(4+3*obj.grid_size^2:end,2:end).*repmat(W,3,1)+0.05.*repmat(W,3,1);
      obj.weights_xy([2 3+3*obj.grid_size^2],2:end) = 0;
      obj.weights_xy([3:2+3*obj.grid_size^2  4+3*obj.grid_size^2:end],1) = 0;
      obj.weights_xy(1,2:end) = .5*mean(obj.weights_xy(1,2:end));
...
%         (rand(obj.n_inputs, obj.ny_normal))- obj.xy_weight_range;
      ...
      try
      obj.weights_yz(2:end,:) = obj.yz_weight_range .* diag(ones(1,obj.nz));
      catch
          keyboard
      end
%       keyboard;
