classdef CTMemoNetwork < handle
  ...
  properties
    ...
    xy_weight_range = .20; % Range [-a,a] of initial weights
...
%     old_ZS;
  ...
  methods % Public methods
    ...
    function resetTraces(obj)
      ...
      obj.wyx_traces = zeros( obj.ny, obj.nx );
      obj.wyz_traces = zeros(obj.ny + obj.bias_hidden, obj.nz);
      obj.dinpy_dym_trace = zeros( obj.ny, obj.n_inputs );
...
%       obj.wzzs_traces= zeros(obj.nzs , obj.nz);
      ...
      obj.prev_y = zeros(2, obj.ny);
      obj.prev_x = zeros(1, obj.n_inputs);
...
%       obj.old_ZS = -ones(1,obj.nzs); 
  ...
  methods (Access=protected) % Protected methods
    ...
    function init_weights(obj) % Initialize weights
      ...
      x_fpn = 2;
      x_fpm = 3+3*obj.grid_size^2;
      
      x_grid_n = 3:2+3*obj.grid_size^2;
      x_grid_m = 4+3*obj.grid_size^2:obj.n_inputs*2+1;
      y_grid_n = 2:obj.ny_normal;
      y_grid_m = 2+obj.ny_normal:obj.ny_normal*2;
      obj.weights_xy = obj.xy_weight_range * rand(obj.n_inputs*2+1, obj.ny_normal*2);%- obj.xy_weight_range;
      obj.weights_xy(x_grid_n,y_grid_n) = obj.weights_xy( x_grid_n,y_grid_n).*repmat(W,3,1)+0.02.*repmat(W,3,1);
      obj.weights_xy( x_grid_m,y_grid_m) = obj.weights_xy( x_grid_m,y_grid_m).*repmat(W,3,1)+0.02.*repmat(W,3,1);
      obj.weights_xy(x_fpn,2:end) = 0;
      obj.weights_xy(x_fpm,[1:obj.ny_normal obj.ny_normal+2:end])=0;
      obj.weights_xy(x_grid_n, y_grid_m) = 0;
      obj.weights_xy(x_grid_m, y_grid_n) = 0;
      
      
      obj.weights_xy([x_grid_n  x_grid_m],[1 obj.ny_normal+1]) = 0;
      obj.weights_xy(1,2:end) = mean(obj.weights_xy(1,2:end));
      obj.wxy_class = 0*obj.weights_xy;
      for col = 1:3
        obj.wxy_class( 3+(col-1)*obj.grid_size^2:2+col*obj.grid_size^2,y_grid_n) = W *2*(col-1)+class_connection;
        obj.wxy_class( 4+(col+2)*obj.grid_size^2:3+(col+3)*obj.grid_size^2,y_grid_m) =  W *2*(col+2)+ class_connection;
      end
      if obj.use_class_connections
          for conn_type = 1:12
              obj.weights_xy(obj.wxy_class==conn_type) = mean(obj.weights_xy(obj.wxy_class==conn_type));
          end
      end
...
%         (rand(obj.n_inputs, obj.ny_normal))- obj.xy_weight_range;
      ...
      obj.weights_yx = obj.weights_xy';
      obj.weights_yx(:,1) = [];
      obj.weights_yx(1,:) = 0;
      obj.weights_yx(:,1:end/2)=0;
      obj.weights_yx(1:end/2, end/2+1:end) = obj.weights_yx(end/2+1:end, end/2+1:end);
      obj.weights_yx = (abs(obj.weights_yx)>0).*rand(size(obj.weights_yx,1),size(obj.weights_yx,2)).*obj.xy_weight_range*2;
      obj.wyx_class = obj.wxy_class';
      obj.wyx_class(:,1) = [];
      
      obj.wyx_class(1:end/2, end/2+1:end) = obj.wyx_class(1:end/2, 1:end/2);
      obj.wyx_class(:,1:end/2)=0;
      if obj.use_class_connections
          for conn_type = 1:12
              if mod(conn_type,2)==1
                 obj.weights_yx(obj.wyx_class==conn_type) = mean(obj.weights_yx(obj.wyx_class==conn_type));%obj.weights_yx(obj.wyx_class==conn_type) = 0;
              else
                  obj.weights_yx(obj.wyx_class==conn_type) = mean(obj.weights_yx(obj.wyx_class==conn_type));
              end
          end
      end
      ...
      try
      obj.weights_yz(2:obj.ny/2+1,:) = obj.yz_weight_range .* diag(ones(1,obj.nz));
      obj.weights_yz(obj.ny/2+2:end,:) = obj.yz_weight_range .* diag(ones(1,obj.nz));
      obj.weights_yz(obj.ny/2+2,:) = 0;
      catch
          keyboard
      end
%       keyboard;
