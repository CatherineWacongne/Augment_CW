...
subplot(1,2,1);imagesc(-log10(P_anov)); ylabel('unit');xlabel('color'); title({'modulation of hidden memory units response'; 'by position of the target (avova: -log10(p))'})
colorbar
...
figure('Color', [1 1 1], 'Position', [70 489 912 420]);
subplot(1,2,1); imagesc(squeeze(sum(H(81:95,:,:),3))); title({'Number of positions for which the activity of memory hidden units'; 'is up regulate if a target of a given color is present'})
set(gca,'XTickMode','auto','XTickLabel', {'Red', 'Green' 'Blue'}, 'XTick', [ 1 2 3] ); ylabel('Memory Unit')
subplot(1,2,2); imagesc(DA./repmat(mean(DA,2),1,3)); title({'Normalized activity of memory units during the delay'; 'in function of the cue color'})
...
%%
w_xy = n.weights_xy;
memo_units_reorder = [2 9 10 13 3 5 7 12 15 1 4 6 8 11 14];
...
figure('Color', [1 1 1]); imagesc([mean(Wreord(:,1:4),2) mean(Wreord(:,5:9),2) mean(Wreord(:,9:15),2)],[-2 2])
line([1.5 1.5], [0.5 54])
line([2.5 2.5], [0.5 54])
line([-0.5 15.5], [3.5 3.5])
line([-0.5 15.5], [10.5 10.5])
