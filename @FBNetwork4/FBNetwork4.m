...
%     
  ...
  methods % Public methods
    ...
    function resetTraces(obj)
      ...
      obj.wzzs_traces= zeros(obj.nzs + obj.bias_hidden, obj.nz);
...
%       obj.Y_ma_total = zeros(1, obj.ny_memory);
    ...
    function init_network(obj) 
      ...
      if (strcmp(obj.input_method, 'modulcells'))
        ...
        obj.ny = obj.ny_normal*2;% + obj.ny_memory;
      else
        % Set dummy activations:
    ...
    function setInstantTransform(obj,type, varargin)
      ...
      switch lower(type)
       ...
       case 'rectified-linear'  
         obj.hd_transform_normal = @(in) log(1+exp(in));
         obj.hd_transform_normal_deriv = @(in) 1 ./ (1  + exp(-in));
