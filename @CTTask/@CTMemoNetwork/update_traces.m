...
% sum_t_xd = obj.sum_t_xd; 
    ...
    w_km = obj.weights_xy(n_k_pre ,n_m_post);
...
%     obj.wyz_traces(obj.weights_yz==0)=0;
    if obj.use_class_connections
        ...
        b = 0*obj.wyz_traces;
        b(3:obj.ny_normal+1,2:end) = a;
        b(obj.ny_normal+3:end,2:end) = a*2;
        obj.wyz_traces(b==1) = mean(obj.wyz_traces(b==1));
        obj.wyz_traces(b==2) = mean(obj.wyz_traces(b==2));
    end
    
    
    dPdwkm = obj.sum_t_xd' * (dsig_m .*  w_ma');
    obj.wxy_traces(n_k_pre, n_m_post) = obj.wxy_traces(n_k_pre, n_m_post) + dPdwkm;
%     dPdwkm =  x_k' * (dsig_m .*  w_ma');
%     obj.wxy_traces(n_k_pre, n_j_post) = obj.wxy_traces(n_k_pre, n_j_post) + dPdwkm;
...
%     keyboard
    ...
    obj.wxy_traces_now = obj.wxy_traces;
    
%     keyboard;

    % trace hidden to modul
    % 1/ update the dinpy_dwyk
    
    A = (ones(1,obj.ny_memory) *w_km').*dsig_k; 
    obj.dinpy_dym_trace = obj.dinpy_dym_trace + obj.prev_y(1,:)'*(A.*x_i) - obj.prev_y(2,:)'*(A .* obj.prev_x); 
    dPdwjk =  ((dsig_m .*  w_ma')* ones(obj.ny_memory,1)).*obj.dinpy_dym_trace;
