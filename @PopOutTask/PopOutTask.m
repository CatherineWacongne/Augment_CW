classdef PopOutTask < handle & Task
    %%%%%%%%%%%%%%%%%%%%%%
    % PopOut search task
    %%%%%%%%%%%%%%%%%%%%%%%%
   properties(SetAccess=protected, GetAccess=public)
        ...
        prev_intTrialType = -1;
        fp_input_index = 17; % index of the input vector that represents the fixation point
        fix_action_index = 1;
    ...
    methods
        ...
        function obj = PopOutTask()
            ...
            obj.target_pos = 0;
            obj.target_col = 0;
            obj.fp_input_index = 17; % index of the input vector that represents the fixation point
        ...
        function setDefaultNetworkInput(obj)
            obj.nwInput = zeros(1, obj.n_col*obj.n_pos+1);   % Fix + 8 positions*2colors(R,G)
        end
                ...
                case obj.FIXSTATE % wait for the fixation to last obj.fix_dur and displays target at the end
                    ...
                    if (~all(networkAction == fixation)) % Trial failed
                        ...
                        if (obj.counter == obj.fix_dur) % Fixated enough
                            ...
                            % Show display:
                            for p = 1:8
                                if obj.display_col(p)>0
                                    obj.nwInput((obj.display_col(p)-1)*obj.n_pos+p)=1; % bring the targets
                                end
                            end   
...
%                     end
        ...
        function setTrialType(obj, target_col, target_pos)
            ...
            obj.distr_col = 3-obj.target_col;
            
            obj.display_col = zeros(1,8);
            while numel(find(obj.display_col==obj.distr_col)<2)
                obj.display_col = obj.distr_col*(randi(2,1,8)-1);
                obj.display_col(obj.target_pos)=obj.target_col;
            end

            
            obj.intTrialType = (obj.target_col-1)*obj.n_pos + obj.target_pos;
        ...
        function pickTrialType(obj)
            ...
            if (~obj.trialSetExternal && ~obj.keepTrialsforGeneralization)
                obj.target_col = randi(obj.n_col);            
            ...
            elseif (~obj.trialSetExternal && obj.keepTrialsforGeneralization) 
                ...
                disp_col = zeros(1,7);
                while numel(find(disp_col>0))<2
                    disp_col = zeros(1,7);
                    trial_code = obj.train_trials(randi(numel(obj.train_trials)));
                    obj.target_col = floor(trial_code/1e4);
                    obj.target_pos = floor((trial_code-obj.target_col*1e4)/1e3);
                    obj.distr_col = 3-obj.target_col;
                    
                    bin_distr = num2str(dec2bin(mod(trial_code,1000),7));
                    d_pos = 1:obj.n_pos;
                    d_pos(obj.target_pos)=[];
                    
                    for d = 1:numel(bin_distr)
                        disp_col(d)=obj.distr_col *str2num(bin_distr(d));
                    end
                end        
                obj.display_col = zeros(1,8);
                obj.display_col(obj.target_pos)=obj.target_col;
                
                try obj.display_col(d_pos)= disp_col;catch; keyboard; end
% % % %                 
            end
        ...
        function setTrialTypeGen(obj,trial_code)
            ...
            while numel(find(disp_col>0))<2
                disp_col(randi(7))=obj.distr_col;
            end

            obj.display_col = zeros(1,8);
            obj.display_col(obj.target_pos)=obj.target_col;
            obj.display_col(d_pos)= obj.distr_col;
        ...
        function setTrialsForGeneralisation(obj)
            ...
            indices = zeros(1,(obj.n_col)^(obj.n_pos)*obj.n_pos);
            ind=1;
            for c=1:obj.n_col
                for p = 1:obj.n_pos
                    for d = 1:2^(obj.n_pos-1)
