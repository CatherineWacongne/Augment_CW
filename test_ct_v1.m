...
%
if fb
    n = CT_DeepNet1();% CT_MultHiddenNetwork();%CTNetwork();% n = CTMemoNetwork2();
...
for i=1:epochs % an epoch is a point in time, a trial contains multiple epochs (defined by the CAPITALSTATES properties of the task)
    if ~ProgramReady
        ...
        if (trialend)
            ...
            trialno = trialno + 1;
            b = find(trial_ends==1);
            %                  if  max_length == 5 &&  min_length == 4 &&(trialno-toc)>200 && correct_perc(i-1) > .8
            %                     min_length = 5;
            %                     toc = trialno;
            %                     n.beta = 0.01;
            %
            %                 end
            if  max_length == 6 &&  (trialno-toc)>200 && correct_perc(i-1) > .88
...
%                 tac = i;
            ...
            if  max_length == 6 &&  min_length == 2 &&(trialno-toc)>200 && correct_perc(i-1) > .8
                max_length = 6;
                min_length = 3;
                n.exploit_prob = 0.975;
                n.beta = 0.01;
...
%                 keyboard
            ...
            %
%             if  max_length == 4 &&  min_length == 2 &&(trialno-toc)>200 && correct_perc(i-1) > .80
%                 %                     keyboard
%                 
%                 n.beta = 0.02;
%                 min_length = 2;
...
%             end
            %
                            if  max_length == 5 && ~t.turnoff_fp && (trialno-toc)>200 && correct_perc(i-1) > .8
%                                 keyboard
                                
                                max_length = 6;
                                toc = trialno;
                            end
                            if  max_length == 5 && (trialno-toc)>200 && correct_perc(i-1) > .8
%                                 keyboard
                                n.exploit_prob = 0.96;
                                n.beta = 0.02;
                                t.turnoff_fp=0;
...
%                             if  max_length == 6 && (trialno-toc)>200 && correct_perc(i-1) > .6
% %                                 keyboard
%                                 n.exploit_prob = 0.80;
%                                  n.beta = 0.05;
% %                                 max_length = 4;
%                                 toc = trialno;
%                             end
            %                 if  n.beta>0.1 && (trialno-toc)>200 && correct_perc(i-1) > .85
            % %                     keyboard
            %                     toc = trialno;
            % %                     n.beta = 0.1;
            %                     n.exploit_prob = 0.96;
...
save(['resultsCTDeep_' num2str(seed) '.mat'], 'n', 't', 'gamma', 'beta', 'lambda', 'rewards')