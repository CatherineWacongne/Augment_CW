...
%
if fb
    n = CT_MultHiddenNetwork();%CTNetwork();% n = CTMemoNetwork2();
    n.grid_size = 5;
...
% n_trials (estimated number of completed trials)
    ...
    n_trials = 100000*25;%0-10000*60*color_on;
...
%                      trial_res(trialno) = 0.5;
                ...
                trialno = trialno + 1;
                b = find(trial_ends==1);
                if  max_length == 4 && (trialno-toc)>200 && correct_perc(i-1) > .8
                    keyboard 
                 ...
                 if  max_length == 3 && (trialno-toc)>200 && correct_perc(i-1) > .9
                    keyboard 
                ...
                if  n.beta>0.01 && (trialno-toc)>200 && correct_perc(i-1) > .9
%                     keyboard 
                    toc = trialno;
                    n.beta = 0.005;
...
% %                 
%                  if n.beta>0.04 && (trialno-toc)>200 && correct_perc_color(i-1) > .5 
% %                     keyboard 
%                     
%                     n.beta = 0.04;
%                     toc = trialno;
%                 end
% %                 if t.active_tracing && t.distr_curve_on  && correct_perc(i-1) > .90 && (trialno-toc)>200
% % %                      keyboard
% %                     t.active_tracing = 0;
% %                     n.beta = 0.05;
% %                    toc = trialno;
% %                 end
% % %                 if ~t.distr_curve_on &&  t.curve_length>2 && correct_perc(i-1) > .70 && (trialno-toc)>1000
% % %                     t.distr_curve_on =1;
%                     n.beta = 0.1;
%                     toc = trialno;
%                     
...
% % %                 if t.curve_length==3 && correct_perc(i-1) > 0.8
% % % %                         toc = trialno;
% % % %                         
% % % %                         t.active_tracing = 0;
% % %                         t.curve_length=4;
...
% % % %                     
% % %                 end
% % %                 if t.curve_length==2 && correct_perc(i-1) > 0.9
% % % %                         toc = trialno;
% % % %                         
% % % %                         t.active_tracing = 0;
% % %                         t.curve_length=3;
...
save('160721_resultsCT2.mat', 'n', 't', 'gamma', 'beta', 'lambda', 'rewards')