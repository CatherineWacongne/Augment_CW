...
t = CTTaskDeep;
t.trialSetExternal=1;
t.mem_dur = 0;
t.active_tracing = 0;
t.distr_curve_on =1;
n.beta = 0;
n.exploit_prob = 1;
SaveDir = 'D:\PostDoc\Modeling\AuGMEnT_Matlab\Figures_CTDeep\';
%%
max_curve_length = 8;
close all
mod_target_activations = zeros(2, 8, 6, time_steps, max_curve_length,3);
mod_distr_activations =  zeros(2, 8, 6, time_steps, max_curve_length,3);
hid_target_activations = zeros(2, 8, 6, time_steps, max_curve_length,3);
hid_distr_activations =  zeros(2, 8, 6, time_steps, max_curve_length,3);
Q_target_activation = zeros(2, 8, 6, time_steps, max_curve_length);
Q_distr_activation = zeros(2, 8, 6, time_steps, max_curve_length);
...
for length = 3:8
    for dist = 1:6
        ...
        curve1 = 2+grid_size*[1:length];
        curve2 = 3+dist+grid_size*[1:length];
        keyboard
        for cond = 1:2
            switch cond
                case 1
                    t.setTrialType(curve1, curve2)
                case 2
                    t.setTrialType(curve2, curve1)
            end
            ...
            for i = 1:time_steps
                ...
                [new_input, reward, trialend] = t.doStep(action);if i>3;plot_displayDeep;end
                modul = reshape(n.Xmod(1:n.grid_size^2*3),n.grid_size,n.grid_size,3);
                ...
                for c = 1:3
                    modul_c = modul(:,:,c);
                    hidden_c = hidden(:,:,c);
                    mod_target_activations(cond,length, dist,i,1:length,c) = modul_c(curve1);
                    mod_distr_activations(cond,length, dist,i,1:length,c) = modul_c(curve1);
                    hid_target_activations(cond,length, dist,i,1:length,c) = hidden_c(curve1);
                    hid_distr_activations(cond,length, dist,i,1:length,c) = hidden_c(curve1);
                end
                Q_target_activation(cond,length, dist,i,1:length) = Q(curve1);
                Q_distr_activation(cond,length, dist,i,1:length) = Q(curve1);
                if trialend
                    if reward>0.7
                        success = success+1;
                    end
