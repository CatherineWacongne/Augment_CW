...
function [net_success, input_acts, hidden_acts,q_acts, fig_hnd, pretty, trial_types, reward_trials ] = test_fg_nw(nw)


pretty = false;
%trial_type_labels = {'R-1','R-2','R-3','R-4','R-5','R-6','R-7','G-1','G-2','G-3','G-4','G-5','G-6','G-7','B-1','B-2','B-3','B-4','B-5','B-6','B-7'};
%trial_types = [233 234 235 236 243 244 245 246 253 254 255 256  263 264 265 266 1233 1234 1235 1236 1243 1244 1245 1246 1253 1254 1255 1256 1263 1264 1265 1266];%[[ones(7,1);2*ones(7,1);3*ones(7,1)] repmat([2:8]', 3,1)];%[1 2; 1 3; 1 4; 1 5]; % p-l;p-r;a-l;a-r
 trial_types = [333 334  335 343 344 345 353 354 355 1333 1334 1335  1343 1344  1345 1353 1354 1355];

states = zeros(numel(trial_types), 50);
input_acts = zeros(numel(trial_types), 50, nw.n_inputs*3);
hidden_acts = zeros(numel(trial_types), 50,nw.ny);
q_acts = zeros(numel(trial_types), 50, nw.nz);
...
for i = 1:numel(trial_types)
    ...
    %t.setTrialType(trial_types(i,1), trial_types(i,2));setTrialType(obj, targ_size, target_pos, Fig_col)
    trial_type_str = num2str(trial_types(i));
    Fig_col = floor(trial_types(i)/1e3);
    targ_size = str2num(trial_type_str(end-2));
    target_pos = [str2num(trial_type_str(end-1)) str2num(trial_type_str(end))];
    
    t.setTrialType(targ_size, target_pos, Fig_col)
    ...
    figure;subplot(1,2,2);imagesc(reshape(q_acts(i,6,2:101),10,10));
    subplot(1,2,1);imagesc(reshape(input_acts(i,6,1:100),10,10)+reshape(input_acts(i,6,101:200),10,10));
