...
for f = 1:obj.n_hidden_features
    ...
    % Input to hidden:
    obj.v1{f} = obj.v1{f} + obj.beta * obj.fdelta * obj.v1_traces{f}; % Weights from input to hidden layer1
    obj.t1{f} = obj.t1{f} + obj.beta * obj.fdelta * obj.t1_traces{f}; % Weights from modul input to hidden layer 1
    obj.u1{f} = obj.u1{f} + obj.beta * obj.fdelta * obj.u1_traces{f}; % Weights from hidden layer 1 to modul input
    obj.u1mod{f} = obj.u1mod{f} + obj.beta * obj.fdelta * obj.u1mod_traces{f};% Weights from modul hidden 1 to modul input
    for f2 = 1:obj.n_hidden_features
        obj.v2{f}{f2} = obj.v2{f}{f2} + obj.beta * obj.fdelta * obj.v2_traces{f}{f2}; % Weights from hidden layer 1 to hidden layer 2
        obj.t2{f}{f2} = obj.t2{f}{f2} + obj.beta * obj.fdelta * obj.t2_traces{f}{f2}; % Weights from modul hidden1 to hidden layer 2
        obj.u2{f}{f2} = obj.u2{f}{f2} + obj.beta * obj.fdelta * obj.u2_traces{f}{f2};% Weights from hidden layer 2 to modul hidden layer 1
    end
    obj.w{f} = obj.w{f} + obj.beta * obj.fdelta * obj.w_traces{f};
